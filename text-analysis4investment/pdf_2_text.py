...
class PDFProcessor:
    ...
    def drop_empty_cols(self, data):
        ...
        filtered_data = [
            col for col in transposed_data if any(cell != '' for cell in col)
        ]
        return list(map(list, zip(*filtered_data)))
...
all_text_files = glob.glob('./rawtxts/*.txt')


def process_file(file_path):
    save_path = './rawtxts/' + file_path.split('/')[-1].replace('.pdf', '.txt')
    try:
        if save_path not in all_text_files:
            # print('start ', file_path)
            processor = PDFProcessor(file_path)
            processor.process_pdf()
            processor.save_all_text(save_path)
    except Exception:
        print(f'{save_path} needs to be checked')
